...
3. **`classify` is always `Mutating`.** Both bare and typed-arg refuse mid-turn — the picker eventually submits a `UserAction::Resume`, and allowing it mid-stream would interleave a session swap with an in-flight response, which is exactly what the read-only fast path was designed to rule out.
