...
10. **Read-only modals don't bind Enter.** `KvOverview::handle_key` consumes every key; Esc / Ctrl+C cancel universally at the stack layer. Enter is reserved for commit semantics in `ListPicker`-based modals — binding it to dismiss in `KvOverview` would create an asymmetric meaning for the same gesture across modal types.
