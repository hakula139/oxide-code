...
- **DCS pass-through inside tmux.** tmux only forwards a small whitelist of OSC numbers (52, 4, 7, 9, 12) to the outer terminal; OSC 8 isn't on it and gets silently dropped. When `$TMUX` is set, `write_status_hyperlinks` wraps the entire envelope (DECSC + per-link CUP / OSC 8 / replayed cells + DECRC) in `\x1bPtmux;...\x1b\\` with every inner ESC doubled. tmux strips the DCS and forwards the inner bytes verbatim to the outer terminal. Without this wrap, ctrl+click on `#NN` does nothing inside tmux even though OSC 8 works fine in the same terminal outside tmux.
