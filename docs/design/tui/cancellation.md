...
1. **Drop-the-future cancellation, no in-loop checks.** Esc / Ctrl+C drops the `agent_turn` future from the agent loop, and reqwest's SSE-close-on-drop and `kill_on_drop(true)` clean up automatically — simpler than Codex's protocol message.
...
6. **`Status` is a projection of `App` state, not a separate machine.** The variants (`Idle | Streaming | ToolRunning { name } | Cancelling | ExitArmed { until }`) derive from app fields, so `set_active_status` can short-circuit while `Cancelling` / `ExitArmed` without a parallel state to keep in sync.
