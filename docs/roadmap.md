...
- Catppuccin Mocha theme, tool-call blocks with per-tool icons and success / error indicators, GitHub-style Edit diffs with line-number gutters, styled Read excerpts, grouped Grep results, multi-line input, and a status bar with model, working directory, and streaming spinner.
...
- Structured file lists for `glob` to round out per-tool result rendering.
