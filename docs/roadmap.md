...
- AI-generated session titles — on the first user prompt of a fresh session, a detached tokio task asks `claude-haiku-4-5` for a concise 3-7 word sentence-case title and appends it as a new `Entry::Title { source: AiGenerated }`. The tail-scan title pickup supersedes the first-prompt fallback automatically; the TUI status bar refreshes live via `AgentEvent::SessionTitleUpdated`. Failures warn-log only — the first-prompt title stays intact.
