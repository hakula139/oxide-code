...
- Resume sanitization on load: strips trailing `thinking`, drops unresolved assistant `tool_use` blocks and orphan user `tool_result` blocks (both halves of a crashed tool turn), drops empty messages, and appends a continuation sentinel if the last message is a user turn of only tool results — keeps the transcript API-valid after mid-turn crashes or JSONL corruption.
