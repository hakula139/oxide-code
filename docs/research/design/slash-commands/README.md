...
5. **Mid-session model swap via `&mut Client` in the agent loop.** The agent loop owns `Client` by value (it's the same shape `/clear` uses for `set_session_id`), so `/model` returns `Action(UserAction::SwitchModel(id))` and the `SwitchModel` arm calls `client.set_model(id)`. Per-request paths re-read `&self.config.model` every call, so betas / `output_config` / `context_management` pick up the swap on the next stream — no `Arc<RwLock<…>>`, no trait changes.
