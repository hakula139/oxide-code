...
Six built-ins ship: `/clear`, `/config`, `/diff`, `/help`, `/init`, `/status`. Each lives in its own `slash/<name>.rs` file implementing `SlashCommand`. Adding one is a new file plus an entry in `BUILT_INS` (alphabetical). The autocomplete popup, `//foo` literal-escape, and the busy-turn dispatch policy (read-only fast-path; mutators / prompt-submit refuse with a system message) all sit on the cross-command surface — see decisions 1–12 below for the contracts each command rides on.
...
Per-command design lives alongside this doc in the same directory as commands earn the depth: see [/clear](clear.md), [/init](init.md). Simple read-only commands (`/config`, `/diff`, `/help`, `/status`) ride the surface decisions above without their own doc.
