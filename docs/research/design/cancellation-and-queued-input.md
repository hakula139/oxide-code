...
4. **Mid-turn drain at the round boundary, not turn-end.** Press Enter while busy appends to a FIFO and renders dim ghost user-messages above the input. The drain fires inside `agent_turn` between the tool-result push and the next `stream_response` — i.e. the same boundary at which Claude Code splices `queued_command` attachments and Codex drains `pending_input`. The model sees the new user text in the very next API request as part of the same multi-step turn, with no abort. This requires plumbing `user_rx` into `agent_turn` (replacing the current `drive_turn` shim that warns-and-drops mid-turn submits) and having the agent loop accumulate incoming `SubmitPrompt`s in a per-turn `Vec<String>`. If the assistant produces a text-only response with no tool calls in the very first round, there is no round boundary to drain at — the queued message falls through to the existing turn-end drain (still reachable via `App::finalize_idle()` after `AgentEvent::TurnComplete`). Phase 1 shipped only the turn-end half.
   ...
   The on-the-wire shape **as shipped**: each queued prompt becomes its own `User` message containing one `ContentBlock::Text` block (no envelope wrapper, no fold into the trailing tool-result `User`). Anthropic accepts consecutive same-role messages, so the request shape `[..., User(tool_results), User(text_1), User(text_2), ...]` is valid; persisting per-prompt keeps resume-side rendering trivial since each drained prompt round-trips as the same `UserMessage` block fresh prompts use.

   Deliberately deviates from Claude Code and OpenCode, both of which wrap mid-turn user text in a `<system-reminder>The user sent the following message: ...</system-reminder>` envelope so the model has a reliable hook to acknowledge the interjection. We bet that consecutive raw user messages would suffice in practice; the cost of skipping the envelope is one fewer transformation step on the persistence side (the display layer has no envelope to recognize and strip when rendering history). The tradeoff may flip if observation shows the model ignoring mid-turn instructions — adding the wrapper at request time without changing what's persisted is the natural follow-up.
...
The decisions intentionally lean toward the _Codex_ model over the _Claude Code_ model on most surfaces (drop-and-discard cancellation, status-bar hint, FIFO queue with header preview, `pending_input` as a per-turn buffer drained at the round boundary) because Codex is the one Rust precedent and its patterns map onto our existing async / actor architecture without reshaping. The wire-shape decision (decision 4 above) deviates from both Claude Code's `queued_command → UserMessage with <system-reminder>` wrapping and OpenCode's mid-loop `<system-reminder>` reload — we ship raw consecutive user messages without the envelope to keep persistence trivial. Claude Code's interrupted-tombstone preservation and `popAllEditable` are richer but require a transcript-edit pass we have not paid for; opencode's server-state machine is a cleaner separation than what we have today but would force the agent loop into a server-actor refactor we have not committed to.
