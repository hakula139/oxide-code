...
**Autocomplete.** `PromptInput.tsx:1130` shows the popup when the buffer starts with `/`. `commandSuggestions.ts` ranks via Fuse.js across name, aliases, parts split on `_-:`, and descriptions (weights: name 3, alias / part 2, description 0.5). Recently-used commands ride higher via `recordSkillUsage()`. Max 5 items rendered; dismissed on Escape or selection. Display: canonical name first (`/resume (continue)`), description right-aligned to a gutter, matched characters in the canonical name rendered bold, plugin commands grouped in a separate section below the built-ins.
...
4. **Two-column popup with matched-char emphasis.** Matches the Claude Code screenshot — left column shows canonical name + parenthesized aliases (`/clear (new, reset)`), right column shows description on a fixed gutter, matched characters bold. Permits `:` in names so a future `/plugin:cmd` namespace doesn't need a parser rewrite. No Fuse.js — surface is small enough for `starts_with` / `contains` with index tracking.
...
7. **Aliases resolve to the canonical command and display inline.** `/clear` is canonical; `/new` and `/reset` are aliases that route to the same impl. Popup and `/help` show one row per command — `/clear (new, reset)` — not three. Filter searches name + aliases; submitting an alias invokes the canonical impl.
...
v1 ships nine commands, eleven invocations: `/help`, `/clear` (aliases `/new`, `/reset`), `/model`, `/status`, `/resume`, `/theme`, `/init`, `/config` (read-only), `/diff`. Eight names match Claude Code; `/diff` is the one Codex-ism kept for ergonomics. Deferred: `/compact` (no summarization call yet), `/cost` (no token persistence yet), `/login` / `/logout` (interactive OAuth), `/config` editor mode, custom markdown commands.
