...
- **Notable**: aggressive memoization (`OffscreenFreeze`, `React.memo` on `LogoHeader`) avoids cascading re-renders. The release-notes-seen counter advances `lastReleaseNotesSeen` config so the logo suppresses on subsequent startups. Mascot poses are static art, not animation.
...
5. **Claude Code's full-screen takeover**. Visually heavy, and eats vertical space the user wants for chat. The welcome should occupy the empty chat region, not steal the whole frame.
