...
- **Pagination**: Lazy with on-demand expansion. Initial fetch 50 sessions (`INITIAL_ENRICH_COUNT = 50`). `onLoadMore()` pulls `visibleCount * 3` more when the focused index approaches the loaded boundary. Footer shows `({focused} of {displayed})` when overflow exists. Metadata-only ("lite log") loads keep the picker instant; full bodies load only on Enter.
...
- **Search**: Client-side, case-insensitive, instantaneous substring on `preview` (first user message, ~50 chars) and `thread_name`. No fuzzy. Server-side `ThreadListParams { source_kinds, archived, cwd, model_providers }` filters up front; the client filter narrows the loaded page.
